...
$(function () {
    ...
    $.form = new function (that) {
        ...
        this.selecter = '.layui-layout-admin>.layui-body>.layui-tab>.layui-tab-content';
        // 刷新当前页面
        ...
        // 在内容区显示视图
        this.show = function (html,contentUrl='') {
            $(this.selecter).html(html);
            this.reInit($(this.selecter));
            if (!loadPage) {
                loadPage = true
                $.menu.getTabMenu(contentUrl)
            }
            setTimeout(function () {
        ...
        this.load = function (url, data, method, callback, loading, tips, time, headers) {
            ...
            $.ajax({
                ...
                beforeSend: function (xhr) {
                    ...
                    return typeof ret === 'object' ? $.msg.auto(ret, time || ret.wait || undefined) : that.show(ret,url);
                }, complete: function () {
                    $.msg.close(index);
                }
            });
    ...
    $.menu = new function (that) {
        that = this;
        //tab功能
        this.getTabMenu = function (contentUrl) {
            layui.use('element', function () {
                var element = layui.element;
                var $ = layui.jquery;
                var active = {
                    tabAdd: function (url, id, name, first) {
                        //新增一个Tab项
                        if (first) {
                            element.tabAdd('menuItem', {
                                title: $('.layui-nav-child>.layui-this>a').attr("data-target-tips"),
                                id: first //规定好的id
                            })
                            element.tabChange('menuItem', first); //切换
                        }else {
                            element.tabAdd('menuItem', {
                                title: name,
                                id: '#' + url + "?spm=" + id //规定好的id
                            })
                            element.tabChange('menuItem', '#' + url + "?spm=" + id); //切换
                        }
                    },
                    tabChange: function (id) {
                        //切换到指定Tab项
                        element.tabChange('menuItem', id); //根据传入的id传入到指定的tab项
                    },
                }
                if (contentUrl && $('.layui-nav-child>.layui-this>a').attr("data-target-tips")) {
                    active.tabAdd('', '', '', contentUrl)
                }
                //当点击有site-demo-active属性的标签时，即左侧菜单栏中内容 ，触发点击事件
                $('.layui-nav-child dd a').on('click', function () {
                    var dataid = $(this);
                    //这时会判断右侧.layui-tab-title属性下的有lay-id属性的li的数目，即已经打开的tab项数目
                    if (dataid.attr("data-menu-node")) {
                        if ($(".layui-tab-title li[lay-id]").length <= 0) {
                            //如果比零小，则直接打开新的tab项
                            active.tabAdd(dataid.attr('data-open'), dataid.attr("data-menu-node"), dataid.attr("data-target-tips"));
                        } else {
                            //否则判断该tab项是否以及存在
                            var isData = false; //初始化一个标志，为false说明未打开该tab项 为true则说明已有
                            $.each($(".layui-tab-title li[lay-id]"), function () {
                                //如果点击左侧菜单栏所传入的id 在右侧tab项中的lay-id属性可以找到，则说明该tab项已经打开
                                if ($(this).attr("lay-id") == '#' + dataid.attr("data-open") + "?spm=" + dataid.attr("data-menu-node")) {
                                    isData = true;
                                    element.tabChange('menuItem', '#' + dataid.attr('data-open') + "?spm=" + dataid.attr("data-menu-node")); //切换
                                }
                            })
                            if (isData == false) {
                                //标志为false 新增一个tab项
                ...
                element.tabChange('menuItem', layid); //假设当前地址为：http://a.com#test1=222，那么选项卡会自动切换到“发送消息”这一项

                element.on('tab(menuItem)', function () {
        ...
        this.listen = function () {
            ...
            (function ($menu, miniClass) {
                // Mini 菜单模式切换及显示
                if (layui.data('admin-menu-type')['type-min']) {
            ...
            this.syncOpenStatus = function (mode) {
                $('[data-submenu-layout]').map(function (node) {
                    ...
                    //修改默认不全部展开，只展开第一个菜单
                    else if (layui.data('menu')[node] === 2) $(this).addClass('layui-nav-itemed');
    ...
    $body.on('click', '[data-modal]', function () {
        let width = $(this).attr('data-width') ? $(this).attr('data-width') : '800px';
        let height = $(this).attr('data-height') ? $(this).attr('data-height') : '800px';
        area = [width, height];
        return $.form.modal($(this).attr('data-modal'), 'open_type=modal', $(this).attr('data-title') || $(this).text() || '编辑',area);
    });
    ...
    $body.on('click', '[data-action]', function () {
        ...
        var loading = $this.attr('data-loading'),
            method = $this.attr('data-method') || 'post';
        ...
        if (!$this.attr('data-confirm')) {
            if ($this.attr('data-prompt')) {
                layer.prompt({title: $this.attr('data-prompt')}, function (text, index) {
                    layui.layer.close(index);
                    data.prompt = text;
                    $.form.load(action, data, method, false, load, tips, time);
                });
            } else $.form.load(action, data, method, false, load, tips, time);
        } else {
