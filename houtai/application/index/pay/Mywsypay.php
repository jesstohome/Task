...
class Mywsypay extends PayBase
{
    const PAY_URL = 'https://crsqi.mywsypay.com/ty/orderPay';
    const PAYOUT_URL = 'https://strsu.mywsypay.com/withdraw/singleOrder';
    ...
    public function getConfig($param)
    {
        if ($param == 'busi_code') {
            ...
            if ($type > 0) {
                return config('pay.mywsypay.t' . $type . '.' . $param);
            }
        }
        return config('pay.mywsypay.' . $param);
    }
    ...
    public function createPay(array $op_data): array
    {
        ...
        $log_file = APP_PATH . 'pay_mywsypay.log';
        file_put_contents($log_file, date('Y-m-d H:i:s') . ': POST DATA ' . json_encode($data, JSON_UNESCAPED_UNICODE) . "\n", FILE_APPEND);
        $res = $this->_post(self::PAY_URL, $data, 'json');
        file_put_contents($log_file, date('Y-m-d H:i:s') . ': RES DATA ' . $res . "\n", FILE_APPEND);
