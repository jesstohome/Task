...
class Crontab extends Controller
{
    ...
    public function freeze_order()
    {
        ...
        if ($oinfo) {
            foreach ($oinfo as $v) {
                ...
                //冻结余额
                $userData = Db::table("xy_users")->find($v['uid']);
                if($userData['balance'] < $v['num']){
                    $money = $userData['balance'];
                    $balance = 0;
                    $freeze_balance = $userData['freeze_balance'] + $money;
                }else{
                    $money = $v['num'];
                    $balance = $userData['balance'] - $v['num'];
                    $freeze_balance = $userData['freeze_balance'] + $v['num'];
                }
                Db::table("xy_users")->where(['id'=>$v['uid']])->update(['balance'=>$balance ,'freeze_balance'=>$freeze_balance]);
    ...
    //利息宝结算
    public function lixibaojiesuan()
    {
        $now = time();
