...
class Callback extends Controller
{
    ...
    public function doposit_mbitpay()
    {
          ...
          error_reporting( 0 );
        file_put_contents( dirname( __FILE__ ).'/deposit_post.txt', var_export($_POST, true), FILE_APPEND );
        file_put_contents( dirname( __FILE__ ).'/deposit_get.txt', var_export($_GET, true), FILE_APPEND );
        file_put_contents( dirname( __FILE__ ).'/deposit_input.txt', file_get_contents("php://input"), FILE_APPEND );
        echo 'success';
...
file_put_contents( dirname( __FILE__ ).'/log_input.txt', file_get_contents("php://input"), FILE_APPEND );
        ...
        if( $data['state'] == 1 )
        {
            $sign = md5( $data['state'].$data['merchantNum'].$data['orderNo'].$data['amount'].config('pay.mbitpay.secret'));
            if( $sign == $data['sign'])
            {
                ...
                if (!$oinfo) 
                {
                 ...
                 if (floatval($data['actualPayAmount']) != floatval($oinfo['num'])) 
                 {
                        file_put_contents($log_file, $data['orderNo'] . ' ======金额不对!' . "\n", FILE_APPEND);
                        http_response_code(500);
                        exit;
                 }
                 
                  $data['out_trade_no'] = $data['orderNo'];
                  if ($oinfo['status'] != 1) {
