...
class Send extends Controller
{
    ...
    public function sendsms(Request $request)
    {
        ...
        if ($result['status'] == 1) {  //发送成功
            ...
            if ($r)
                return json(['code' => 0, 'info' => yuylangs('fscg')]);
            else
                return json(['code' => 0, 'info' => yuylangs('fssb')]);
        } else
            return $result;
    }
    ...
    public function smsbao($tel, $code, $g_code)
    {
        ...
        //$data['content'] = yuylangs('ndyzm') . "{$code}," . yuylangs('yzmwfzyx');
        $data['content'] = yuylangs('ndyzm') . "{$code},";
        $res = $this->send_post($url, $data);
        $res = json_decode($res, true);
        if ($res['code'] == '0') {
            return ['status' => 1, 'msg' => yuylangs('fscg')];
        } else {
