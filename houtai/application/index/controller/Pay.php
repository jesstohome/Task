...
class Pay extends Base
{
    ...
    private function _op($payType)
    {
        ...
        if (!$dbRes) {
            $this->showMessage(yuylangs('czsbqshcs'));
        }
    ...
    public function user_ok()
    {
        ...
        if (!$recharge) {
            return $this->error(yuylangs('recharge_u_no_order'));
        }
        if ($recharge['status2'] == 1) {
            return $this->error(yuylangs('qbycftj'));
        }
        ...
        if (!$res) {
            $this->error(yuylangs('czsb'));
        }
        $this->success(yuylangs('with_q_ok'));
    }
    ...
    public function bit($sn = '')
    {
        ...
        $this->pay_info = Db::name('xy_pay')->where('name2', 'bit')->find();
         ...
         $this->info = Db::name('xy_pay')->find(input("id"));
        
        $pay = Db::table("xy_pay")->find(8);
        $recharge = Db::name('xy_recharge')->where('id', $sn)->find();
        if ($recharge['num2'] == 0) {
            $amoutd=$this->op_data['amount'];
            $infopay=$this->pay_info['mch_id'];
          
            $this->op_data['amount2'] = round($recharge['num']/$pay['mch_id'],2);
    ...
    public function sepropay()
    {
        ...
        if ($resData['respCode'] != 'SUCCESS') {
            return $this->error(yuylangs('czsbqshcs'), $resData);
        }
    ...
    public function yulupay()
    {
        ...
        if ($resData['code'] != '1000') {
            return $this->error(yuylangs('czsbqshcs'));
        }
    ...
    public function _empty($name)
    {
        ...
        if ($resData['respCode'] != 'SUCCESS') {
            echo '<h4 style="text-align: center">' . yuylangs('czsbqshcs') . '</h4>' . "\n";
