...
class NodeService
{
    ...
    public static function full($node = null)
    {
        ...
        $node = self::parseString(trim($node, " /"));
        if (substr_count($node, '/') == 1) {
            $node = \request()->module() . '/' . $node;
        }
        return $node;
    }
     ...
     * @throws \think\exception\DbException
     */
    public static function getMenuNodeTree1()
    {
        $list = Db::name('SystemMenu')->where("url != '' && url != '#' && status = 1")->order('sort desc,id asc')->select();
        return self::buildMenuData(Data::arr2tree($list), self::getMethodList());
    }
    ...
    private static function buildMenuData($menus, $nodes)
    {
        foreach ($menus as $key => &$menu) {
            ...
            elseif (preg_match('/^http?\:/i', $menu['url'])) continue;
            elseif ($menu['url'] === '#') unset($menus[$key]);
            else {
                $node = join('/', array_slice(explode('/', preg_replace('/[\W]/', '/', $menu['url'])), 0, 3));
                $menu['url'] = url($menu['url']) . (empty($menu['params']) ? '' : "?{$menu['params']}");
                $re = self::checkAuth($node);
                if (!$re) unset($menus[$key]);
            }
        }
        return $menus;
    }
